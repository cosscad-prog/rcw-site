...
> ⚠️ **이 문서의 손으로 올리는 절차는 옛 방식이다.** 지금은 `publish-trial.ps1` 이
> 업로드부터 사이트 링크 갱신·커밋·푸시까지 한 번에 한다. 아래 화면 설명은 저장소를
> 처음 만들 때와 스크립트가 하는 일을 이해할 때만 참고할 것.
>
> 바뀐 점 두 가지: 파일명에 **버전이 들어가고**(2026-07-29, 그래서 사이트 링크를
> 릴리스마다 고쳐야 한다 — 스크립트가 한다), 파일이 **언어별로 갈리지 않는다**
> (2026-07-30, 설치 파일 하나에 한국어·영어가 함께 들어 있다). 그래서 올리는 파일은
> 8개가 아니라 에디션 2 × Rhino 2 = **4개**다.

## ⚠️★ 트라이얼 설치본은 **빌드일 + 30일** 이면 신규 사용자에게 죽는다

`CAdm_Check_Keys.cs` 의 `ValidateLicense()` 첫 설치 분기:

```csharp
TimeSpan sinceRelease = DateTime.Now - BuildInfo.BuildDate;   // BuildDate.cs
...
`publish-trial.ps1` 이 발행 끝에 `BuildDate.cs` 를 읽어 **기한과 남은 일수**를 찍는다
(10일 이하면 경고). 5.1.3(빌드 2026-08-23)의 기한은 **2026-09-22** 다.

※ 체험 기간(Core 90일 / Standard 30일) 자체는 재설치로 **늘어나지 않는다** — 시작일이
여섯 곳에 남고 그중 **가장 이른 날짜**를 쓴다. 새 버전 안내 메일에 "기간은 그대로
이어집니다" 라고 써도 된다.

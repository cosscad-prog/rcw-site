...
| `MAIL_FROM` 은 **헤더 From 만** 바꾸고 봉투는 계정 그대로 | 봉투(MAIL FROM)를 바꾸면 Gmail 이 거부한다 |

### ⚠️ 못 잡는 것
**Gmail 이 헤더 From 을 갈아 끼우는지**는 여기서 안 보인다. 가짜 서버는 우리가 보낸 그대로를
받아 적을 뿐이다 — 실제로 온 메일의 보낸사람 칸을 눈으로 봐야 안다.

...
### 5. 제품 문의 · Trial 신청 알림 (같은 봇, 2026-08-11 추가)

홈페이지의 **제품 문의(`contact.html`)와 Trial 신청(`trial.html`)도** 같은 텔레그램 봇으로 온다.
...
화면은 되돌려받은 값을 쓴다 — 보낸 값을 그대로 쓰면 서버가 새 id 로 저장한 경우
`downloads.request_id` 가 어디도 가리키지 않는 값이 되어 **누가 받아 갔는지 끊긴다**.
다운로드 클릭 기록(`downloads`)만 아직 공개키로 직접 넣는다(내려받기로 넘어가는 중에
`keepalive` 로 쏘는 기록이라 그대로 두었다).

- 전에는 브라우저가 공개키로 `contacts` 에 직접 넣었다. 저장은 됐지만 **Supabase 대시보드를
  열기 전에는 문의가 온 줄을 몰랐다.** 그래서 서버 함수를 한 단계 끼웠다
- 라이선스 알림과 달리 **이메일·연락처를 가리지 않는다.** 알림을 보고 바로 답장해야 하는데
  가려 두면 결국 대시보드를 열어야 한다
- 본문은 자른다(텔레그램 상한 4096자) — 문의 1200자 / Trial 신청의 목적란 1000자.
  전문은 Supabase 에 남는다(`contacts.message` 5000자, `trial_requests.purpose` 2000자)
- 저장이 실패하면 알림도 보내지 않고 화면에 오류 + `beimptech@gmail.com` 안내가 뜬다.
  반대로 **알림만 실패하면 접수는 정상**이다(예외를 안에서 삼킨다)
- honeypot 은 화면과 서버 양쪽에서 본다. 봇에게는 성공한 것처럼 응답하고 저장하지 않는다
